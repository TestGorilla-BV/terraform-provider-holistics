...
- **`/users` list truncates some fields.** The Holistics list endpoint returns a *subset* of the User schema — `title` and `job_title` are populated by `PUT /users/{id}` but omitted by `GET /users`. There's no `GET /users/{id}` to fall back on, so Read uses the list and state ends up with `title=null` even when the user has a title set server-side. The provider deals with this by writing the *plan* back to state after Update instead of the PUT response: writing the richer PUT response would conflict with the `UseStateForUnknown` plan modifier (the planmodifier pins the field to its last-Read value, then the apply diverges from the plan, triggering `Provider produced inconsistent result after apply`). Practical consequence: don't manage `title` or `job_title` from Terraform; if you set them in config, every apply will show drift because Read can't see them.
- **Rate limits.** The Holistics API enforces per-user, per-endpoint quotas — as low as **120 requests/minute** on the standard tier and **20/min** on heavy endpoints. The provider deals with this in two ways:
  - **Retries with Retry-After.** On a `429 Too Many Requests` response, the provider sleeps for the time specified in the `Retry-After` header (both seconds and HTTP-date forms are handled), then retries. Up to 3 retries per request, capped at 60s wait per attempt. After exhausting retries, the original 429 is returned with the `Retry-After` value preserved on the `APIError` so callers can log it.
  - **In-process caching.** Reads against `/users`, `/users/me`, and `/tags` are cached for 30s within a single `terraform` invocation. This is the primary fix for configs with many `data "holistics_user"` blocks — instead of one `/users` request per data source, the provider fetches the list once and serves all subsequent reads from memory. The cache is invalidated whenever a write (`InviteUser`, `UpdateUser`, etc.) modifies the relevant collection, so resources see fresh data after they change it.
